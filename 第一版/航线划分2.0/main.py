...
def div_one_route(departure_time,arrival_time,time_loc,meteo,schedule,index = 0,point_num=100,DPI=1000):
    ...
    map = Basemap(llcrnrlon=lonmin - 0.5, llcrnrlat=latmin - 0.5, urcrnrlon=lonmax + 0.5, urcrnrlat=latmax + 0.5,
                  resolution='f', projection='lcc', lat_0=(latmax + latmin) / 2,
...
def divide_all_routes(iship_path,meteo_path):
    ...
    for i in range(len(schedule)):
        ...
        div_one_route(departure_time, arrival_time, time_loc, meteo, schedule, index=i + 1, DPI=1000)


if __name__ == '__main__':
    iship_path = 'iship_working.csv'
    meteo_path = 'meteo_working(1).csv'
